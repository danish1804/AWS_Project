...
public class SubscriptionService {
    ...
    public static Map<String, Object> getSubscriptions(Map<String, String> params, DynamoDbClient ddb) {
        try {
            System.out.println("✅ [Subscription] getSubscriptions() called");
            ...
            for (Map<String, AttributeValue> item : queryResponse.items()) {
                ...
                try {
                    GetItemRequest musicRequest = GetItemRequest.builder()
                            .tableName("music")
                            .key(Map.of("title", AttributeValue.fromS(title), "album", AttributeValue.fromS(album)))
                            .build();

                    GetItemResponse musicResp = ddb.getItem(musicRequest);
                    if (musicResp.hasItem() && musicResp.item().containsKey("image_s3_url")) {
                        song.put("image_s3_url", musicResp.item().get("image_s3_url").s());
                    }
                } catch (Exception e) {
                    System.err.println("⚠️ Failed to fetch image_url for: " + title + " - " + album);
                    e.printStackTrace();
            ...
            return buildJsonResponse(200, successBody);
    ...
    private static Map<String, Object> buildJsonResponse(int statusCode, Map<String, Object> body) {
        Map<String, Object> response = new HashMap<>();
        response.put("statusCode", statusCode);
        response.put("headers", getCorsHeaders());
